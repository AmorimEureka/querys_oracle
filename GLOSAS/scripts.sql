...
WITH WIEW_API AS (
  SELECT
        ...
        COALESCE(
            g.nr_guia,
            MAX(g.nr_guia) OVER (
                PARTITION BY a.cd_paciente, a.cd_atendimento, rf.cd_remessa, rf.cd_convenio
            )
        ) AS nr_guia,
        g.cd_senha,
        a.nr_carteira,
    ...
    SELECT
        ...
        COALESCE(
            g.nr_guia,
            MAX(g.nr_guia) OVER (
                PARTITION BY a.cd_paciente, a.cd_atendimento, ra.cd_remessa, ra.cd_convenio
            )
        ) AS nr_guia,
        g.cd_senha,
        a.nr_carteira,
...
SELECT
    ...
    COUNT(DISTINCT CD_REMESSA) AS QTD_REMESSA,
    SUM(vl_total_registro) AS vl_total_registro,
    SUM(vl_total_conta) AS vl_total_conta
FROM WIEW_API
WHERE
    cd_convenio = 10 AND
    sn_pertence_pacote ='N' AND
    -- TO_CHAR(dt_competencia, 'YYYY') = '2026'
    -- TO_CHAR(dt_competencia, 'MM/YYYY') = '06/2026'
     nr_guia = '622374'
GROUP BY
    ds_fatura,
...
FROM (
    ...
    SELECT
        ...
        'TOTAL GERAL' AS ds_fatura,
        SUM(qtd_remessa) AS qtd_remessa,
        SUM(vl_total_registro) AS vl_total_registro,
        SUM(vl_total_conta) AS vl_total_conta,
        1 AS ordem
    FROM RETORNO
...
SELECT * FROM dbamv.HPC_V_CONTA_ATENDIMENTO WHERE CD_ATENDIMENTO = 285444;
SELECT * FROM dbamv.HPC_V_CONTA_ATENDIMENTO WHERE CD_REMESSA = 17413;
SELECT * FROM dbamv.HPC_V_CONTA_ATENDIMENTO WHERE nr_guia = '622374' ;
SELECT * FROM dbamv.HPC_V_CONTA_ATENDIMENTO WHERE TO_CHAR(DT_COMPETENCIA, 'MM/YYYY') = '03/2026';
