...
WITH PACIENTE_DIA
    AS (
        -- VALIDADA C/ RELATORIO 'R_CENSO'
        SELECT

            EXTRACT(MONTH FROM ai.DT_ATENDIMENTO) AS MES ,
            EXTRACT(YEAR FROM ai.DT_ATENDIMENTO) AS ANO ,
            CASE
                WHEN ui.DS_UNID_INT LIKE '%POSTO%' THEN
                    'POSTO'
                ELSE ui.DS_UNID_INT
        ...
        WHERE EXTRACT(YEAR FROM ai.DT_ATENDIMENTO) = EXTRACT(YEAR FROM SYSDATE) AND
              ai.TP_ATENDIMENTO IN( 'I', 'U') AND
              p.NM_PACIENTE NOT LIKE '%TEST%' AND
              ( COALESCE(ai.DT_ALTA, SYSDATE) - ai.DT_ATENDIMENTO ) > 1 -- REMOVENDO PACIENTES HOSPITAL-DIA
            --   ai.DT_ALTA IS NULL AND
            -- CASE WHEN ui.DS_UNID_INT LIKE '%POSTO%' THEN 'POSTO' ELSE ui.DS_UNID_INT END = 'UTI 1'
        GROUP BY
...
MOVIMENTACAO_INTERNAS
    AS (
        -- VALIDADA C/ RELATORIO 'R_MOV_UNID_INT'
        SELECT
...
TREATS
    AS (
        SELECT
            ...
            CASE
                WHEN TRUNC( pd.QTD_PACIENTE_DIA  / COALESCE( ( mi.SAI_TRANSFPARA + pa.QTD_ALTAS ), 1) ) = 0 THEN 1
                ELSE TRUNC( pd.QTD_PACIENTE_DIA  / COALESCE( ( mi.SAI_TRANSFPARA + pa.QTD_ALTAS ), 1) )
            END AS TEMPO_MEDIO
...
WHERE ptd.CD_TIPO_DOCUMENTO NOT IN (36, 44)
	  AND doc.CD_DOCUMENTO IN ('939')
	  AND DBMS_LOB.SUBSTR(erc.LO_VALOR, 4000, 1) IS NOT NULL
      AND pdc.CD_ATENDIMENTO = 238014 ; --242448 ;




-- ANALISE:
-- O CAMPO 'LO_VALOR' DA TABELA 'EDITOR_REGISTRO_CAMPO' ARMAZENA OS VALORES DOS CAMPOS DO DOCUMENTO
...
SELECT
    ...
    CASE
        WHEN EXTRACT(MONTH FROM a.DT_ATENDIMENTO) = 1 THEN 'Jan'
        WHEN EXTRACT(MONTH FROM a.DT_ATENDIMENTO) = 2 THEN 'Fev'
        WHEN EXTRACT(MONTH FROM a.DT_ATENDIMENTO) = 3 THEN 'Mar'
        WHEN EXTRACT(MONTH FROM a.DT_ATENDIMENTO) = 4 THEN 'Abr'
        WHEN EXTRACT(MONTH FROM a.DT_ATENDIMENTO) = 5 THEN 'Mai'
        WHEN EXTRACT(MONTH FROM a.DT_ATENDIMENTO) = 6 THEN 'Jun'
        WHEN EXTRACT(MONTH FROM a.DT_ATENDIMENTO) = 7 THEN 'Jul'
        WHEN EXTRACT(MONTH FROM a.DT_ATENDIMENTO) = 8 THEN 'Ago'
        WHEN EXTRACT(MONTH FROM a.DT_ATENDIMENTO) = 9 THEN 'Set'
        WHEN EXTRACT(MONTH FROM a.DT_ATENDIMENTO) = 10 THEN 'Out'
        WHEN EXTRACT(MONTH FROM a.DT_ATENDIMENTO) = 11 THEN 'Nov'
        WHEN EXTRACT(MONTH FROM a.DT_ATENDIMENTO) = 11 THEN 'Dez'
    END AS NOME_MES,
...
WHERE
    ...
    pdc.TP_STATUS NOT IN('ABERTO', 'CANCELADO') --AND
    -- pdc.CD_ATENDIMENTO = 241070 --238014 -- 241034
GROUP BY
	pdc.CD_ATENDIMENTO,
    pdc.CD_PACIENTE,
    CASE
...
SELECT
	...
	, pdc.DH_CRIACAO
	, pdc.CD_USUARIO AS CD_USUARIO
FROM DBAMV.PW_DOCUMENTO_CLINICO pdc WHERE pdc.CD_ATENDIMENTO = 241034;
...
LEFT JOIN  DBAMV.EDITOR_CAMPO eca           ON eca.CD_CAMPO = erc.CD_CAMPO
INNER JOIN DBAMV.ATENDIME a                 ON pdc.CD_ATENDIMENTO = a.CD_ATENDIMENTO
WHERE pdc.CD_ATENDIMENTO = 215780 AND
      ptd.CD_TIPO_DOCUMENTO NOT IN (36, 44) AND
	  doc.CD_DOCUMENTO IN ('939')
;

-- 241034 - CICERA MOREIRA ALEXANDRE PERES  -  SEM CONSELHO-CE: 1093565
-- 203340 - MARIA LINDONETE ALVES - CREFITO-CE: 277960
-- 184142 - MARIA LINDONETE ALVES - CREFITO-CE: 277960
-- 192150 - YANDRA QUIXADA DIAS CARLOS - CREFITO-CE: 309644
-- 215780 - MARIA LINDONETE ALVES - CREFITO-CE: 277960
