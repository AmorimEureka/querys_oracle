...
TREATS
    AS (
        ...
        WHERE tcam.DH_PROCESSO > TRUNC(ADD_MONTHS(SYSDATE, -1), 'MM')
),
AGRUPAMENTOS
    AS (
        SELECT
...
SELECT
    ...
    QTD
FROM AGRUPAMENTOS
    -- WHERE NM_PRESTADOR = 'RICARDO SANTANA PARENTE SOARES JUNIOR' AND CLASSIFICACAO_PROCESSO = 'TA Consulta' AND ORDEM_PROCESSO = 4 AND NM_CONVENIO = 'IPM' AND NM_USUARIO = 'LARA LAYSA ARAUJO DUARTE'
ORDER BY
    TIPO,
    DT,
    TP_ATENDIMENTO,
